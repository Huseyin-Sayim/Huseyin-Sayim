<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($isim_err) && empty($yazar_err) && empty($inceleme_err) && empty($cat_err) && empty($tarih_err) && empty($resim_err) && empty($durum_err) && empty($puan_err)) {
            ...
            $myPath = SITE.'img/Kitap/'. $file_name;
...
<body>
    <form action="<?=SITE?>/index.php?page=kitap_ekle.php" method="POST" enctype="multipart/form-data" class="w-75 float-end mx-auto my-5 border p-3">
        <div class="mb-3">
    ...
    <?php include '../views/ck_editor.php'; ?>
</body>
</html>