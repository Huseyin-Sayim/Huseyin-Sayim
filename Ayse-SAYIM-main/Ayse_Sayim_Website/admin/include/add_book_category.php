<?php
    $category = $messaege = $match = '';
    $category_err = $added_success = '';
    if (isset($_POST['add_cat']) && !empty($_POST['category'])) {
        ...
        if (!empty($match)) {
            ...
            $_SESSION['type'] = 'error';
            echo '<script>window.location.href = "'. SITE .'index.php?page=add_book_category.php ";</script>';
        } else {
...
<form action="#" method="POST" class="w-50 mx-auto my-5">
        ...
        <input type="text" name="category" id="category" class="form-control mb-3 mt-1" placeholder="Eklemek istediğiniz kategoriyi giriniz" >
        ...
        <input type="submit" name="add_cat" value="Ekle" class="btn btn-secondary form-control text-uppercase fw-bold">
        <?php 
            if (!empty($messaege)) {
                echo $messaege;
            }
